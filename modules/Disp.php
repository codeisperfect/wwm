...
<?php
	...
	public static function disp_formelm($name,$elm){
		if($elm["type"]=="mselect"){
			if($elm["design"]==1)
				load_view("template/mselect.php",array("labels"=>$elm["options"],"selectall"=>false,"name"=>$name,"data"=>array(),"label"=>$elm["name"]));
		}
		else if($elm["type"]=="text"){
			form_input(array('name'=>$name,"lname"=>$elm["name"],"dc"=>$elm["dc"]));
		}
		else if($elm["type"]=="select"){
			load_view("template/select.php",array('name'=>$name,"label"=>$elm["name"],"dc"=>$elm["dc"],"options"=>arr2option($elm["options"])) );
		}
		else if($elm["type"]=="select_bool"){
			$elm=Fun::mergeifunset($elm,array("options"=>array("Yes","No")));
			load_view("template/select_bool.php",array('name'=>$name,"label"=>$elm["name"],"options"=>$elm["options"]));
		}
		else if($elm["type"]=="textarea"){
			form_textarea(array("lname"=>$elm["name"],"name"=>$name));
		}
	}
	public static function disp_form($form,$name){
	?>
	<form onsubmit="form.sendreq(this,$(this).find('button[type=submit]')[0]);" >
	<?php
		foreach($form as $i=>$val){
			Disp::disp_formelm("ms_".$name."_".$i,$val);
		}
